...
const wrapper = ({children}) => <FlagsmithProvider environmentId={MY_ENV_ID}>{children}</FlagsmithProvider>


describe('useFlagsmith', () => {

    test('makes sure flagsmith is initialized', async() => {
        jest.spyOn(flagsmith, 'init').mockImplementationOnce(() => Promise.resolve())

        const {result, waitForNextUpdate} = renderHook(() => useFlagsmith(), {wrapper})
    ...
    test('handles when flagsmith has an error during initialization', async () => {
        jest.spyOn(flagsmith, 'init').mockImplementationOnce(() => Promise.reject())
    ...
    test('an invalid identify user sets isIdentified to false', async () => {
        const identify = jest.spyOn(flagsmith, 'identify').mockReturnValue(Promise.reject())
    ...
    test('you can check if a feature exists and get it using hasFeature and getValue', () => {
        ...
        const hasFeature = jest.spyOn(flagsmith, 'hasFeature').mockImplementationOnce((v) => !!features[v])
        const getValue = jest.spyOn(flagsmith, 'getValue').mockImplementationOnce((v) => features[v])
    ...
    test('you can get all flags using getFlags', async () => {
        ...
        expect(await result.current.getFlags()).toEqual({repo: 'flagsmith-react'})
    })
    ...
    test('you can increment or set a trait providing its key and value', async() => {
        ...
        const incrementTrait = jest.spyOn(flagsmith, 'incrementTrait').mockImplementationOnce((k, v) => ({
            ...traits,
            [k]: traits[k] + v
        }))
        const setTrait = jest.spyOn(flagsmith, 'setTrait').mockImplementationOnce((k, v) => ({
            ...traits,
            [k]:  v
        }))
        const setTraits = jest.spyOn(flagsmith, 'setTraits').mockImplementationOnce((v) => v)

        const {result} = renderHook(() => useFlagsmith(), {wrapper})

        expect(await result.current.setTrait('repo', 'flagsmith-react')).toEqual({stars: 1, repo: 'flagsmith-react'})
        expect(setTrait).toHaveBeenCalledWith('repo', 'flagsmith-react')

        expect(await result.current.incrementTrait('stars', 3)).toEqual({stars: 4})
        expect(incrementTrait).toHaveBeenCalledWith('stars', 3)

        expect(await result.current.setTraits({ repo: 'flagsmith-react'})).toEqual({ repo: 'flagsmith-react'})
    ...
    test('you can get and set traits using getTrait and setTraits', () => {
        ...
        const getTrait = jest.spyOn(flagsmith, 'getTrait').mockImplementationOnce((v) => traits[v])
